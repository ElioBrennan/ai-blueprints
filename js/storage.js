...
md += `\`\`\`\n`;
  ...
  _generateLLMContext(data) {
    ...
    md += `## ${t('llm.projectInfo')}\n\n`;
    md += `- **${t('export.tableName')}**: ${data.name || t('app.untitled')}\n`;
    md += `- **${t('export.nodesLabel')}**: ${data.nodes ? data.nodes.length : 0}\n`;
    md += `- **${t('export.edgesLabel')}**: ${edges.length}\n`;
    md += `- **${t('llm.exportDate')}**: ${new Date().toISOString().split('T')[0]}\n`;
    const langLabel = getCurrentLang ? getCurrentLang() : 'es';
    md += `- **${t('llm.lang')}**: ${langLabel}\n\n`;

    md += `## ${t('llm.instructions')}\n\n`;
    md += `${t('llm.instructionsText')}\n\n`;

    md += `> ${t('llm.topologicalNote')}\n\n`;
